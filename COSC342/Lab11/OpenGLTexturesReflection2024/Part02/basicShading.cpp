	...
	if(res){
		...
		while( glfwGetKey(window, GLFW_KEY_ESCAPE ) != GLFW_PRESS && glfwWindowShouldClose(window) == 0 ){// Clear the screen
			...
			if( glfwGetKey(window, GLFW_KEY_1) ==GLFW_PRESS){
				std::cout<<"change light colour to red" <<std::endl;
				shader->setLightColour(glm::vec3(1.0,0,0));
			}else if( glfwGetKey(window, GLFW_KEY_2) ==GLFW_PRESS){
				std::cout<<"change light colour green" <<std::endl;
				shader->setLightColour(glm::vec3(0,1.0,0));
			}else if( glfwGetKey(window, GLFW_KEY_3) ==GLFW_PRESS){
				std::cout<<"change light colour blue" <<std::endl;
				shader->setLightColour(glm::vec3(0,0,1.0));
			}
			// Clear the screen
