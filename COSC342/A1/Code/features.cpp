...
int runTesting(cv::Mat image1, cv::Mat image2, cv::Ptr<cv::DescriptorMatcher> matcher, std::vector<double>& error_projection_list, std::vector<double>& inlier_ratio_list, std::vector<int>& inliers_count_list, std::vector<int>& total_match_list, std::string type);
template <typename T>
...
int main(int argc, char *argv[])
{
	...
	std::vector<double> flann_inlier_ratio_list;
	std::vector<int> flann_inliers_count_list;
	std::vector<int> flann_total_match_list;
	std::vector<double> bf_error_projections_list;
	std::vector<double> bf_inlier_ratio_list;
	std::vector<int> bf_inliers_count_list;
	std::vector<int> bf_total_match_list;
	for (int i = 1; i < 105; i++)
	{
		...
		runTesting(image1, image2, cv::BFMatcher::create(), bf_error_projections_list, bf_inlier_ratio_list, bf_inliers_count_list, bf_total_match_list, "BF");
		runTesting(image1, image2, cv::FlannBasedMatcher::create(), flann_error_projections_list, flann_inlier_ratio_list, flann_inliers_count_list, flann_total_match_list, "FLANN");
		keypoints1.clear();
		keypoints2.clear();


	}
	writeVectorToCSV(flann_error_projections_list, "flann_error_projections_with_outliers.csv");
	writeVectorToCSV(flann_inlier_ratio_list, "flann_inlier_ratio.csv");
	writeVectorToCSV(flann_inliers_count_list, "flann_inliers_count_list.csv");
	writeVectorToCSV(flann_total_match_list, "flann_total_match_list.csv");
	writeVectorToCSV(bf_error_projections_list, "bf_error_projections_with_outliers.csv");
	writeVectorToCSV(bf_inlier_ratio_list, "bf_inlier_ratio.csv");
	writeVectorToCSV(bf_inliers_count_list, "bf_inliers_count_list.csv");
	writeVectorToCSV(bf_total_match_list, "bf_total_match_list.csv");
...
int runTesting(cv::Mat image1, cv::Mat image2, cv::Ptr<cv::DescriptorMatcher> matcher, std::vector<double>& error_projection_list, std::vector<double>& inlier_ratio_list, std::vector<int>& inliers_count_list, std::vector<int>& total_match_list, std::string type)
{
	...
	inlier_ratio_list.push_back(inlierCount / (float)goodMatches.size());
	inliers_count_list.push_back(inlierCount);
	total_match_list.push_back(goodMatches.size());
